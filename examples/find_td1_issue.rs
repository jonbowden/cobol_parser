...
fn main() {
    ...
    // Find first line that introduces an error
    for end_line in 180..=220 {
        ...
        if let Some(tree) = parser.parse(&normalized, None) {
            ...
            if errors > 0 {
                println!("First error when including line {}", end_line);
                println!("Line content: {}", source.lines().nth(end_line - 1).unwrap_or(""));
                break;
            }
        }
    }
