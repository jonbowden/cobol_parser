...
fn preprocess_line(
    ...
    // Check both the whole line (for standard format) and code area (for lines with change markers)
    let trimmed = line.trim();
    let upper_trimmed = trimmed.to_uppercase();
    let code_trimmed = code_area.trim().to_uppercase();
    if upper_trimmed == "EJECT"
        || upper_trimmed.starts_with("SKIP")
        || upper_trimmed == "EJECT."
        || code_trimmed == "EJECT"
        || code_trimmed.starts_with("SKIP")
        || code_trimmed == "EJECT."
    {
        // Convert to a comment line
        return (format!("      * {}", code_area.trim()), fixes, warnings);
    }
